...
(async () => {
    ...
    await concat(files, './dist/digipad/v' + version + '/digipad.min.js');

    const data = fs.readFileSync('./dist/digipad/v' + version + '/digipad.min.js');
    const fd = fs.openSync('./dist/digipad/v' + version + '/digipad.min.js', 'w+');
